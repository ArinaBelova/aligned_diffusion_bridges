...
from proteins.docking.dataset import build_data_loader
from proteins.docking.models import build_model_from_args

from sbalign.training.epoch_fns import train_epoch_sbalign, test_epoch_sbalign, inference_epoch_docking
...
from sbalign.utils.setup import wandb_setup, parse_docking_train_args, update_args_from_config
...
def train(args, train_loader, val_loader, model, optimizer, scheduler, ema_weights=None, log_dir=None):
    ...
    g = get_diffusivity_schedule(args.diffusivity_schedule, args.max_diffusivity,K=args.K,H=args.H,norm=args.norm)
    loss_fn = loss_fn_from_args(args)

    logs = {'val_loss': math.inf, "val_inference_rmsd": math.inf}

    rot_tr_file = os.path.join(log_dir, "R_tr.pt")
    ...
    for epoch in range(args.n_epochs):
        ...
        if args.inference_every > 0 and (epoch + 1) % args.inference_every == 0:
            traj_dict, inference_metrics = inference_epoch_docking(
                                        model=model, g=g, orig_dataset=val_loader.dataset,
                                        num_inference_complexes=args.num_inference_complexes,
                                        inference_steps=args.inference_steps,
                                        samples_per_complex=args.samples_per_complex,
                                        rot_vec=rot_tr_dict['rot_vec'],
                                        tr_vec=rot_tr_dict['tr_vec']
                                    )
...
def main(cmd_args=None):
    ...
    print(flush=True)
    args = parse_docking_train_args(cmd_args=cmd_args)
    ...
    # Datasets
    train_loader, val_loader, rot_tr_dict = build_data_loader(args)
    ...
    if args.log_dir is not None:
        ...
        rot_tr_file = os.path.join(log_dir, "R_tr.pt")
        print(f"Saving rot-tr to {rot_tr_file}", flush=True)
        torch.save(rot_tr_dict, rot_tr_file)
