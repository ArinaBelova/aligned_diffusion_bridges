...
def parse(args, is_train=True):
    ...
    # Now it is in config_train and config_test
    args.is_train = is_train

    scale = 1
    if args.distortion == 'sr':
        scale = args.degradation.scale

        ##### sr network ####
        args.network_G.setting.upscale = scale
        # opt["network_G"]["setting"]["in_nc"] *= scale**2

    # datasets
    for phase, dataset in args.datasets.items():
        phase = phase.split("_")[0]
        dataset["phase"] = phase # will be set like train/val/test
        dataset["scale"] = scale # just always 1 if not super-resolution task or testing regime  
        ...
        if dataset.get("dataroot_LQ", None) is not None:
            #dataset["dataroot_LQ"] = osp.expanduser(args.dataroot_LQ)
    ...
    # path: if we want to train in chunks 
    ...
    # what about validation? validation is inside the training - yes it is
    # TODO: do we want the same structure of saved artifacts as in imagerec?
    # if is_train:
    ...
    # print("TYPE OF OUR ARGS IS: ", type(args))
